...
$\underline{Notations}$. Let $(\Omega,\mathcal{F},\mathbb{P})$ be a probability space, $(E,\mathcal{E})$ be a measurable space. Expectations of random variable $X:\Omega\mapsto E$ can be interpreted as integrals with respect to $\mathbb{P}$. Let $\mu$ be the distribution of $X$. We can interpret $\mu$ as the image of $\mathbb{P}$ under $X$. For event $A\in \mathcal{E}$, we write the probability that $X$ is in $A$ as \begin{equation}
\mu(A) = \mathbb{P}(X^{-1}A) = \mathbb{P}(X\in A) = \mathbb{P}\{\omega \in \Omega:X(\omega) \in A\}.
\end{equation} Note that $X^{-1}A$ denotes the event for every $A$ in $\mathcal{E}$. i.e., $X^{-1}A = \{X\in A\} = \{\omega \in \Omega:X(\omega) \in A\}$. It also suggests that $X$ is measurable relative to $\mathcal{F}$ and $\mathcal{E}$. 
...
\mathbb{P}(X=0) &= \mathbb{P}(X\in A)= \mathbb{E}[\mathbb{1}_A \circ X] = \mathbb{E}\big[\mathbb{E}[\mathbb{1}_{A}\circ X|\pi]\big] \nonumber \\
